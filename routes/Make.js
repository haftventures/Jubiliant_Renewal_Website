...
router.get('/make_status', allowRoles(1,2), async (req, res) => {
  try {
...
router.post('/make_verify_details', allowRoles(1,2), async (req, res) => {
  try {
    ...
    const result = await apiCaller.apicallerLivePort('make/make_verify_details', payload);
...
router.post('/make_verify_action', allowRoles(1,2), async (req, res) => {
  try {
    ...
    const payload = {
      ...
      stage_position: 1  //stage details 1 for make verification
    };
